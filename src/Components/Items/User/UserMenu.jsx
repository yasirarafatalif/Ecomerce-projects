...
import { User, Settings, LogOut, Package, Heart, ChevronRight } from "lucide-react";
    ...
    <div className="relative hover:cursor-pointer" ref={menuRef}>
      ...
      <button
        ...
        className={`p-2.5 rounded-full transition-all active:scale-95 ${
          isOpen ? "bg-black text-white" : "bg-[#1A1A1A] text-white hover:bg-black"
        }`}
      >
      ...
      {isOpen && (
        <div className="absolute right-0 mt-3 w-64 bg-white shadow-2xl border border-gray-100 py-4 z-[60] animate-in fade-in slide-in-from-top-2 duration-200">
          ...
          <div className="px-5 pb-4 border-b border-gray-100 mb-2">
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                <span className="text-[13px] font-black uppercase tracking-tighter">Yasir Arafat</span>
                <span className="text-[10px] font-bold text-gray-400 lowercase">yasir@xiv.com</span>
              </div>
            </div>
          </div>
          ...
          {/* Links Section */}
          <div className="flex flex-col">
            <DropdownLink icon={<Package size={14} />} label="My Orders" to="/my-orders" />
            <DropdownLink icon={<Heart size={14} />} label="Wishlist" to="/wishlist" />
            <DropdownLink icon={<Settings size={14} />} label="Account Settings" to="/account-settings" />
            
            <hr className="my-2 border-gray-50" />
            
            <button className="flex items-center gap-3 px-5 py-2.5 text-red-500 hover:bg-red-50 transition-colors w-full text-left">
              <LogOut size={14} />
