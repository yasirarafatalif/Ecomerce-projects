...
const CollectionsPage = () => {
  ...
  const categories = ["All", "T-SHIRTS", "JEANS", "JACKETS", "ACCESSORIES", "BEST SELLERS"];
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full xl:w-auto">
          ...
          <Link to="/dashboard/add-product">
            <button className="bg-black text-white px-8 py-4 flex items-center justify-center gap-3 hover:bg-blue-700 transition-all active:scale-95 shadow-2xl">
               <Plus size={16} />
               <span className="text-[10px] font-black uppercase tracking-widest">Add New Piece</span>
            </button>
          </Link>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50/50 border-b border-gray-100">
                <th className="p-8 text-[9px] font-black uppercase tracking-[0.2em] text-gray-400">Product</th>
                <th className="p-8 text-[9px] font-black uppercase tracking-[0.2em] text-gray-400">Details</th>
                <th className="p-8 text-[9px] font-black uppercase tracking-[0.2em] text-gray-400">Stock Status</th>
                <th className="p-8 text-[9px] font-black uppercase tracking-[0.2em] text-gray-400 text-right">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50">
              {filteredProducts.length > 0 ? filteredProducts.map((item) => (
                <tr key={item._id} className="group/row hover:bg-gray-50/80 transition-all duration-300">
                  <td className="p-8">
                    <div className="flex items-center gap-5">
                      <div className="w-16 h-20 bg-gray-100 overflow-hidden relative shadow-sm border border-gray-50 group-hover/row:border-black transition-all duration-500">
                        <img src={item.img} alt={item.title} className="w-full h-full object-cover grayscale-[30%] group-hover/row:grayscale-0 group-hover/row:scale-110 transition-all duration-700" />
                      </div>
                      <div>
                        ...
                        <p className="text-[9px] font-bold text-gray-300 uppercase mt-1 tracking-widest">UID: {item._id.toUpperCase()}</p>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="p-8">
                    <div className="flex items-center gap-3">
                      <div className={`w-2 h-2 rounded-full ${item.stock > 0 ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]' : 'bg-red-500 animate-pulse'}`}></div>
                ...
                <tr>
                  <td colSpan="4" className="p-20 text-center text-[11px] font-black uppercase tracking-widest text-gray-300 italic">
