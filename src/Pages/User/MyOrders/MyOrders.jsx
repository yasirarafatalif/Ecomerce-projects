...
import { Package, Search, ChevronRight, CheckCircle2, Truck, Clock, AlertCircle, ArrowUpRight, Filter } from 'lucide-react';
  ...
  const orders = [
    {
      ...
      payment: "Credit Card",
      items: [
        { name: "Basic Slim Fit T-Shirt", qty: 1, price: "$199", img: "https://images.unsplash.com/photo-1521572163474-6864f9cf17ab?q=80&w=200" },
        { name: "Soft Wash Jeans", qty: 1, price: "$199", img: "https://images.unsplash.com/photo-1541099649105-f69ad21f3246?q=80&w=200" }
      ]
    },
    {
      id: "XIV-2026-002",
      ...
      items: [
        { name: "Embroidered Shirt", qty: 1, price: "$199", img: "https://static.zara.net/photos///2024/V/0/2/p/0679/402/800/2/w/563/0679402800_6_1_1.jpg" }
      ]
    }
  ];
    ...
    <div className="min-h-screen bg-[#F2F2F2] pt-28 pb-20 font-sans">
      <div className="max-w-[1200px] mx-auto px-4 md:px-12">
        ...
        <div className="flex flex-col gap-10">
          {orders.map((order) => (
            <div key={order.id} className="bg-white group transition-all duration-500 hover:shadow-2xl">
              
              {/* Order Info Header */}
              <div className="p-6 md:p-8 flex flex-col md:flex-row justify-between gap-8 border-b border-gray-50">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-8 flex-1">
                  <InfoBlock label="Order Reference" value={`#${order.id}`} />
                  <InfoBlock label="Order Placed" value={order.date} />
                  <InfoBlock label="Payment Method" value={order.payment} />
                  <InfoBlock label="Total Amount" value={order.total} isBold />
                </div>
              ...
              <div className="px-6 md:px-8 py-6 bg-gray-50/50 flex flex-col md:flex-row justify-between items-center gap-6">
                <div className="flex items-center gap-6 text-[10px] font-black uppercase tracking-widest text-gray-400">
                   <span className="flex items-center gap-2 text-green-600"><CheckCircle2 size={14}/> Paid</span>
                   <span className="flex items-center gap-2"><Package size={14}/> {order.status === "Delivered" ? "Delivered to Home" : "At Sorting Center"}</span>
                </div>
        ...
        <div className="mt-20 pt-10 border-t border-gray-200 flex flex-col md:flex-row justify-between items-center gap-6">
           ...
           <div className="flex gap-8 text-[10px] font-black uppercase tracking-widest underline underline-offset-4">
              <a href="#" className="hover:text-blue-700">Need Help?</a>
              <a href="#" className="hover:text-blue-700">Shipping Policy</a>
              <a href="#" className="hover:text-blue-700">Returns</a>
           </div>
        </div>
      </div>
    </div>
...
const InfoBlock = ({ label, value, isBold }) => (
  <div className="flex flex-col gap-1">
    ...
    <span className={`text-[11px] uppercase tracking-tighter ${isBold ? "font-black text-black text-sm italic" : "font-bold text-gray-800"}`}>
      {value}
    </span>
  </div>
);
...
const StatusBadge = ({ status }) => {
  ...
  return (
    <div className={`flex items-center gap-2 px-4 py-1.5 rounded-full border shadow-sm ${
      isDelivered ? "bg-green-50 border-green-100 text-green-700" : "bg-blue-50 border-blue-100 text-blue-700"
    }`}>
